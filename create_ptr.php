...
<body>
    ...
    <script>
        const productMetaByDescription = <?= json_encode($productMetaByDescription, JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT) ?>;
        const batchNumbersByDescription = <?= json_encode($batchNumbersByDescription, JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT) ?>;
        const batchMetaByDescription = <?= json_encode($batchMetaByDescription, JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT) ?>;
        const productMetaByDescriptionLower = Object.keys(productMetaByDescription).reduce(function (acc, key) {
            acc[String(key).trim().toLowerCase()] = productMetaByDescription[key];
            return acc;
        }, {});
        const batchNumbersByDescriptionLower = Object.keys(batchNumbersByDescription).reduce(function (acc, key) {
            acc[String(key).trim().toLowerCase()] = batchNumbersByDescription[key];
            return acc;
        }, {});
        const batchMetaByDescriptionLower = Object.keys(batchMetaByDescription).reduce(function (acc, key) {
        ...
        const ptrNoInput = document.getElementById('ptr_no');
        const recipientInput = document.getElementById('recipient');
        const printPreviewBtn = document.getElementById('printPreviewBtn');
        let batchListCounter = itemRowsBody.querySelectorAll('.item-row').length;
        ...
        function getDescriptionMeta(descriptionValue) {
            const key = String(descriptionValue || '').trim();
            if (key === '') {
                return {
            ...
            const exactProduct = productMetaByDescription[key];
            const exactBatchOptions = batchNumbersByDescription[key];
            const exactBatchMeta = batchMetaByDescription[key];
            if (exactProduct) {
                return {
        ...
        function applyRowMeta(row) {
            ...
            const descriptionValue = descriptionSelect.value.trim();
            ...
            const unitCostInput = row.querySelector('.item-unit-cost');
            const expirationInput = row.querySelector('.item-expiration');
            const meta = getDescriptionMeta(descriptionValue);
            const selectedProduct = meta.selectedProduct;
            const batchOptions = meta.batchOptions;
            const batchMeta = meta.batchMeta;
        ...
        function createItemRow(itemData = {}) {
            ...
            const batchListId = 'batchOptionsList_dynamic_' + batchListCounter++;
            const batchOptions = batchNumbersByDescription[descriptionValue] || [];
            ...
            tr.innerHTML =
                ...
                '<td><input type="date" class="form-control item-expiration" value="' + escapeHtml(itemData.expiration_date ?? '') + '" readonly></td>' +
        ...
        function getEnteredItems() {
            ...
            itemRowsBody.querySelectorAll('.item-row').forEach(function (row) {
                ...
                const batchNumber = row.querySelector('.item-batch-number').value.trim();
                const quantity = row.querySelector('.item-quantity').value.trim();
                const unit = row.querySelector('.item-unit').value.trim();
                const unitCost = row.querySelector('.item-unit-cost').value.trim();
                const amount = row.querySelector('.item-amount').value.trim();
                const program = row.querySelector('.item-program').value.trim();
                const expiration = row.querySelector('.item-expiration').value.trim();
        ...
        function renderPreviewItems(items) {
            ...
            const rowsToRender = items.slice(0, previewLineRows);
            rowsToRender.forEach(function (item) {
                const tr = document.createElement('tr');
                const descriptionWithBatch = item.batch_number
            ...
            for (let i = rowsToRender.length; i < previewLineRows; i++) {
                const tr = document.createElement('tr');
        ...
        function syncRowRequiredState(row) {
            ...
            const batchInput = row.querySelector('.item-batch-number');
            const quantityInput = row.querySelector('.item-quantity');
            const programInput = row.querySelector('.item-program');

            const description = descriptionInput.value.trim();
            const batchNumber = batchInput.value.trim();
            const quantity = quantityInput.value.trim();
            const program = programInput.value.trim();
            const isActiveRow = description !== '' || batchNumber !== '' || quantity !== '' || program !== '';
        ...
        function validateProgramsBeforeNext() {
            ...
            for (let i = 0; i < rows.length; i++) {
                const row = rows[i];
                const state = syncRowRequiredState(row);
        ...
        function fillPreview() {
            const enteredItems = getEnteredItems();
        ...
        nextPreviewBtn.addEventListener('click', function () {
            ...
            if (!fillPreview()) {
                const firstRow = itemRowsBody.querySelector('.item-row');
                if (!firstRow) {
                    return;
                }
        ...
        printPreviewBtn.addEventListener('click', function () {
            fillPreview();
            const previewHtml = document.getElementById('previewPrintArea').outerHTML;
